...
def main():
    ...
    for vendor, state in rb.STATES.items():
        jev, llm, fails, jw, lw = [], [], [], [], []
    ...
    print()
    for name in ("jev", "llm"):
        w = [x for v in rb.STATES for x in out[name][v]["wall_ms"] if x]
        f = sum(len(out[name][v].get("failures", [])) for v in rb.STATES)
        print(f"{name.upper()}: median wall {st.median(w):.0f} ms, p95 {sorted(w)[int(len(w)*0.95)-1]:.0f} ms, "
              f"failures {f}/{a.runs*len(rb.STATES)}")
